...
`alpaka` ships as a PyPI package (`alpaka`). Versioning is automated by
...
| `feat:` | minor | `feat: add streaming client` |
...
| --- | --- | --- |
| `main` | stable `X.Y.Z` | the default install (`pip install alpaka`) |
| `next` | prereleases `X.Y.Z-rc.N` | published as a **prerelease** — only reached via `pip install alpaka --pre` or an exact pin |
| `N.x` (e.g. `1.x`) | maintenance `X.Y.Z` | published stable for an older major |

PyPI marks `…-rc.N` versions as prereleases, so a plain `pip install alpaka`
...
`jhnnsrs/alpaka:${ALPAKA_SERVICE_TAG:-latest}`, and `integration.yaml` sets the same
default. Every branch is therefore tested against `:latest`, the released line.

To test against another published tag, set the repository variable `ALPAKA_SERVICE_TAG`
...
main   ──1.5.2──(merge next)──> 2.0.0 -> 2.0.1 …    (stable PyPI)
          │ cut `1.x` from main HEAD *before* the 2.0.0 merge
1.x    ──1.5.2──> 1.5.3 -> 1.5.4 …                  (stable PyPI for v1)
```
...
3. **Promote v2.** Merge `next` → `main`. The breaking change since `1.5.2`
   makes PSR cut stable `2.0.0`.

## Backporting a fix to v1 (after v2 has shipped)

Branch off `1.x`, PR the fix into `1.x` with a `fix:` commit. PSR cuts `1.5.3`
and publishes it to PyPI. Forward-port the same fix to `main`/`next` if it also
applies there.

## Consuming the next channel

```sh
pip install alpaka --pre          # latest rc (or stable, whichever is newer)
pip install 'alpaka==2.0.0-rc.1'  # pin a specific rc
```

Stable consumers (`pip install alpaka`) are unaffected by the `next` channel.
