...
async def test_deltas_are_coalesced_and_the_message_is_finished_with_the_full_text() -> None:
    ...
    async with rath:
        async with stream_into_room(room="3", agent_id="assistant", flush_chars=10, flush_interval=999) as reply:
...
async def test_a_raising_body_still_finishes_the_message() -> None:
    ...
    with pytest.raises(RuntimeError):
        async with rath:
            async with stream_into_room(room="3", agent_id="assistant", flush_chars=999, flush_interval=999) as reply:
...
async def test_camel_case_and_unset_omission_survive_regeneration() -> None:
    rath, calls = recording_rath()
    async with rath:
        async with stream_into_room(room="3", agent_id="assistant") as reply:
