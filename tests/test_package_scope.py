"""alpaka must stay importable, and correct, without arkitekt.

arkitekt is an optional integration. Two places may know about it: the integration
module ``alpaka/arkitekt.py``, and ``alpaka/__init__.py``, which imports that module
last and behind ``try/except ImportError``. Everything else takes the client it works with explicitly.
"""
...
PACKAGE = Path(alpaka.__file__).parent
...
def test_only_the_integration_module_imports_arkitekt() -> None:
    ...
    assert not offenders, (
        f"{offenders} import arkitekt at module scope. alpaka works without arkitekt: "
        "take the client explicitly instead of importing it."
    )


def test_alpaka_imports_with_arkitekt_unavailable() -> None:
    ...
    script = (
        ...
        "assert hasattr(Alpaka, 'aget_room')\n"
        "print('ok')\n"
    )
...
CLIENT_CLASS = "alpaka.alpaka.Alpaka"

#: turms' dumped configuration, beside the module it generated.
DUMPED_CONFIG = Path(__file__).resolve().parents[1] / "alpaka/api/project.json"
