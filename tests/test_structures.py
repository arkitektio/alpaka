"""alpaka's structures: what it can send across a node boundary, and how.
...
from alpaka import arkitekt as declared  # noqa: E402

#: The identifiers alpaka puts on the wire. Changing one is a cross-service change,
...
def test_merging_stamps_the_service_that_owns_them() -> None:
    ...
    app = AppRegistry()
    app.merge(declared.registry, service="alpaka")

    services = {s.service for s in app.structure_registry.structures()}
    assert services == {"alpaka"}
